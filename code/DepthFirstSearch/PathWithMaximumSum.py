...
Input:  1
   ...
   4  5 6   7
Output: 18
Explanation: The path with the maximum sum is: 5 -> 2 -> 1 -> 3 -> 7
...
Input:  -1
        / \
          -3
Output: -1
Explanation: The path with the maximum sum is: -1
'''
...
class Solution:
    ...
    def findMaximumPathSumOwn(self, root):
        ...
        def dfs(root):
            if not root:
                return 0
            left = max(dfs(root.left), 0) # This is to ignore negative values, 
            # as they will reduce the sum
            right = max(dfs(root.right), 0)
            
            self.globalMaximumSum = max(
                self.globalMaximumSum, left + right + root.val)
